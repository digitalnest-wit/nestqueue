...
export default function TicketsPage() {
  ...
  } = useTickets({ query: searchValue, filter, order });
      ...
      <div className="flex w-full h-[100vh] overflow-hidden transition-all duration-300 ease-in-out">
        <div
          className={`transition-all duration-300 ease-in-out ${
            selectedTicket ? "md:w-1/2 lg:w-2/3" : "w-full"
          } overflow-auto ${
        ...
        >
            ...
            <div className="flex gap-2 items-center">
              ...
              >
                {didSelectFilter ? (
                  <LabeledIcon
                    ...
                    icon={<Funnel fill="currentColor" className="w-4" />}
                    label={filter}
                  />
                ) : (
              ...
              >
                {didSelectOrder ? (
                  <LabeledIcon
                    className="p-1 py-1 text-sm"
                    icon={<ArrowDownNarrowWide className="w-4" />}
                    label={order}
                  />
                ) : (
          ...
          {ticketsError && (
            <div className="px-4 py-2 bg-gray-100 dark:bg-gray-800 dark:text-gray-300 flex gap-3 items-center text-black shadow-md transition-coo">
              {ticketsError.message.includes("404") ? (
                <p>
                  No tickets were found for query '{searchValue}'. Please try
                  another.
                </p>
              ) : (
                <p>An unknown error occurred: {ticketsError.message}</p>
              )}
            </div>
          )}
          {tickets && (
